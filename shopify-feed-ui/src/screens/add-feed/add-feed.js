...
export default class AddFeed extends React.Component {
    state = {
        ...
        skyBlue: '',
        lightPink: '',
        productList: JSON.parse(localStorage.getItem('_product_list'))
    };
    ...
    render () {
        const { productName, tags, salePrice, mrp, itemCost, size, black, white, red, charcoalGrey, goldenYellow, lightPink, melangeGrey, navyBlue, oliveGreen, royalBlue, skyBlue } = this.state;
        const productList = JSON.parse(localStorage.getItem('_product_list'));
        return (
            <div className="add-feed-container">
                ...
                <div className="column product-list">
                    <span className="title"><b>Product List - ( {productList && productList.length+' items'} )</b></span>
                    {productList && productList.map((product, index) => {
                        return (
                            <div className="list-item" key={index}>
                                ...
                                <span>{product.product_name} | {product.tags}</span>
                    ...
                    {productList && productList.length > 0 ?
                        <>
                            <button className="hk-button save clear-all" onClick={this.onClearAll.bind(this)}>CLEAR ALL</button>
    ...
    onRemoveProduct(index) {
        ...
        if (productList) {
            ...
            localStorage.setItem('_product_list', JSON.stringify(productList));
            this.setState({productList});    
        }
    }
    onClearAll() {
        localStorage.removeItem('_product_list');
    ...
    onSaveProduct() {
        ...
        if (productName) {
            ...
            localStorage.setItem('_product_list', JSON.stringify(productListArray));
            this.setState({
                productName: '',
                tags: 'men, women, printed, hoodie',
                salePrice: '799',
